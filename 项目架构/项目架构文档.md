...
| 模块 | 功能描述 |
|------|---------|
| 数据采集 | 多源股票数据下载（东方财富、雪球、Yahoo、通达信等） |
| 技术分析 | MACD、布林带、周期分析等技术指标计算 |
| 深度学习 | CNN模型进行趋势预测和分类 |
| **模拟交易** | 虚拟资金交易，策略验证和回测 |
| **实盘交易** | GUI自动化控制交易软件执行真实交易 |
| 持仓管理 | 账户资金、持仓、盈亏、风险控制 |
...
│   ├── account.py             # 账户资金模型
│   ├── positions.py           # 持仓管理模型
│   ├── trade_records.py       # 交易记录模型
│   └── signals.py             # 交易信号模型
├── routes/trade/              # 交易路由
│   ├── trading_route.py       # 交易操作路由
│   └── trading_strategies.py  # 交易策略路由
├── codes/AutoTrade/           # 自动化交易
│   ├── AutoTrading.py         # 自动交易主逻辑
│   └── autotrading_utils.py   # 自动交易工具函数
└── services/                  # 交易服务
...
位置：`App/models/trade/account.py`

| 字段 | 说明 |
|------|------|
| account_id | 账户唯一标识 |
| account_type | 账户类型（实盘/模拟） |
| total_balance | 总余额 |
...
account.freeze_balance(amount)    # 冻结资金
account.unfreeze_balance(amount)  # 解冻资金

# 统计更新
account.update_position_value(value)  # 更新持仓市值
account.update_trade_statistics(is_winning, pnl)  # 更新交易统计

# 风险检查
account.check_risk_limits()       # 检查风险限制
```

### 4.7.3 持仓模型 (Position)

位置：`App/models/trade/positions.py`
...
```python
# 持仓操作
position.add_position(quantity, price)     # 增加持仓
position.reduce_position(quantity, price)  # 减少持仓
position.update_price(current_price)       # 更新价格

# 风险控制
position.set_stop_loss(price)     # 设置止损
position.set_take_profit(price)   # 设置止盈
position.check_stop_loss()        # 检查止损触发
position.check_take_profit()      # 检查止盈触发
```

### 4.7.4 交易记录模型 (TradeRecord)

位置：`App/models/trade/trade_records.py`
...
**技术实现**：
- 使用 **pywinauto** 控制交易软件窗口
- 使用 **pyautogui** 模拟鼠标键盘操作
- 使用 **OpenCV** 模板匹配定位UI元素

**交易配置**：
```python
class TradingConfig:
    BUY_AMOUNT = 5000        # 单笔买入金额
    MIN_SHARES = 100         # 最小买入股数（1手）
    MAX_PRICE = 200          # 最高价格限制
...
| 模式 | 类 | 说明 |
|------|-----|------|
| 模拟交易 | TradePoolFaker | 虚拟资金交易，用于策略验证 |
| 实盘交易 | TradePoolReal | 真实资金交易，控制交易软件 |
...
信号触发
    ↓
┌─────────────────┐
│  风险检查       │  检查账户余额、持仓比例、止损限制
└────────┬────────┘
         ↓
┌─────────────────┐
│  订单生成       │  创建TradeRecord记录
└────────┬────────┘
         ↓
    ┌────┴────┐
    │         │
